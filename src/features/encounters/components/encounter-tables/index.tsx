...
interface EncounterTableProps {
  data: Encounter[];
  /** Hide the pagination footer (useful for embedded/sub-tables) */
  hidePagination?: boolean;
  /** Column IDs to hide (e.g. ['patient'] when viewing a single patient) */
  hideColumns?: string[];
}
...
export function EncounterTable({
  ...
  const filteredColumns = React.useMemo(
    () =>
      hideColumns?.length
        ? columns.filter(
            (c) => !hideColumns.includes(c.id ?? (c as any).accessorKey ?? '')
          )
        : columns,
    [hideColumns]
  );
  ...
  const table = useReactTable({
    ...
    ...(hidePagination && { initialState: { pagination: { pageSize: 500 } } })
  });

  const hasPatientCol = filteredColumns.some(
    (c) => (c.id ?? (c as any).accessorKey) === 'patient'
  );
